...
(define-syntax tpch-q1
  (er-macro-transformer
    (lambda (form rename compare)
      (let ((lineitem (list-ref form 1))
            (shipdate-bound (list-ref form 2)))
        `(let ((shipdate-bound% ,shipdate-bound))
           (boss-run
             `(OrderBy
                (Project
                  (GroupBy
                    (Project
                      (Filter ,(quote ,lineitem) (LessEqual l_shipdate ,shipdate-bound%))
...
(define-syntax tpch-q2
  (er-macro-transformer
    (lambda (form rename compare)
      (let ((supplier (list-ref form 1))
            (nation   (list-ref form 2))
            (region   (list-ref form 3))
            (partsupp (list-ref form 4))
            (part     (list-ref form 5)))
        `(begin
           (boss-eval (Name
             (Join (Join ,supplier
      (Join ,nation
      (Filter ,region (Equal r_name "EUROPE"))
      (Equal n_regionkey r_regionkey))
      (Equal s_nationkey n_nationkey))
      ,partsupp
      (Equal s_suppkey ps_suppkey))
             q2_supp_europe))
           (boss-eval (Name
             (Project
           ...
           (boss-eval
             (Project
               (Slice
                 (OrderBy
                   (Join (Join (Filter ,part (And (Equal p_size 15) (Like p_type "%BRASS")))
      (ByName q2_supp_europe)
      (Equal p_partkey ps_partkey))
      (ByName q2_min_cost)
      (Equal ps_partkey ps_partkey) (Equal ps_supplycost min_supplycost))
...
(define-syntax tpch-q3
  (er-macro-transformer
    (lambda (form rename compare)
      (let ((customer (list-ref form 1))
            (orders   (list-ref form 2))
            (lineitem (list-ref form 3))
            (cutoff   (list-ref form 4)))
        `(let ((cutoff% ,cutoff))
           (boss-run
             `(Slice
                (OrderBy
                  (Project
                    (GroupBy
                      (Project
                        (Join (Join (Filter ,(quote ,customer) (Equal c_mktsegment "BUILDING"))
      (Filter ,(quote ,orders) (Less o_orderdate ,cutoff%))
      (Equal c_custkey o_custkey))
      (Filter ,(quote ,lineitem) (Greater l_shipdate ,cutoff%))
...
(define-syntax tpch-q4
  (er-macro-transformer
    (lambda (form rename compare)
      (let ((lineitem     (list-ref form 1))
            (orders       (list-ref form 2))
            (orderdate-lo (list-ref form 3))
            (orderdate-hi (list-ref form 4)))
        `(let ((lo% ,orderdate-lo)
               (hi% ,orderdate-hi))
           (begin
             (boss-eval (Name
             ...
             (boss-run
               `(OrderBy
                  (GroupBy
                    (Join (Filter ,(quote ,orders)
                              (And (GreaterEqual o_orderdate ,lo%)
                                   (Less o_orderdate ,hi%)))
  ...
  (er-macro-transformer
    (lambda (form rename compare)
      (let ((supplier     (list-ref form 1))
            (nation       (list-ref form 2))
            (region       (list-ref form 3))
            (customer     (list-ref form 4))
            (orders       (list-ref form 5))
            (lineitem     (list-ref form 6))
            (orderdate-lo (list-ref form 7))
            (orderdate-hi (list-ref form 8)))
        `(let ((lo% ,orderdate-lo)
               (hi% ,orderdate-hi))
           (begin
             (boss-eval (Name
      ...
      (Equal s_nationkey n_nationkey))
             ...
             (boss-eval (Name
               (Project
                 (Join ,customer
      (ByName q5_supp_asia)
      (Equal c_nationkey s_nationkey))
                 c_custkey s_suppkey n_name)
               q5_cust_supp))
             (boss-run
               `(Name
                  (Project
                    (Join (Filter ,(quote ,orders) (And (GreaterEqual o_orderdate ,lo%)
                                                     (Less o_orderdate ,hi%)))
      (ByName q5_cust_supp)
      (Equal o_custkey c_custkey))
                 ...
                 (Project
                   (GroupBy
                     (Project
                       (Join ,lineitem
      (ByName q5_ord_supp)
      (Equal l_orderkey o_orderkey) (Equal l_suppkey s_suppkey))
                       n_name
                       (As (Multiply l_extendedprice (Subtract 1.0 l_discount)) disc_price))
...
(define-syntax tpch-q6
  (er-macro-transformer
    (lambda (form rename compare)
      (let ((lineitem    (list-ref form 1))
            (shipdate-lo (list-ref form 2))
            (shipdate-hi (list-ref form 3)))
        `(let ((lo% ,shipdate-lo)
               (hi% ,shipdate-hi))
           (boss-run
             `(GroupBy
                (Project
                  (Filter ,(quote ,lineitem)
                          (And (GreaterEqual l_shipdate ,lo%)
                               (And (Less l_shipdate ,hi%)
...
(define-syntax tpch-q7
  (er-macro-transformer
    (lambda (form rename compare)
      (let ((nation      (list-ref form 1))
            (supplier    (list-ref form 2))
            (orders      (list-ref form 3))
            (customer    (list-ref form 4))
            (lineitem    (list-ref form 5))
            (shipdate-lo (list-ref form 6))
            (shipdate-hi (list-ref form 7)))
        `(let ((lo% ,shipdate-lo)
               (hi% ,shipdate-hi))
           (begin
             (boss-eval (Name
             ...
             (boss-run
               `(OrderBy
                  (GroupBy
                    (Project
                      (Filter
                        (Join (Join (Join (Join (Filter ,(quote ,lineitem)
                                        (And (GreaterEqual l_shipdate ,lo%)
                                             (LessEqual l_shipdate ,hi%)))
      ,(quote ,supplier)
      (Equal l_suppkey s_suppkey))
      (Join ,(quote ,orders)
      ,(quote ,customer)
      (Equal o_custkey c_custkey))
      (Equal l_orderkey o_orderkey))
      (ByName q7_supp_nation)
      (Equal s_nationkey sn_nationkey))
      (ByName q7_cust_nation)
      (Equal c_nationkey cn_nationkey))
                        (Or
...
(define-syntax tpch-q8
  (er-macro-transformer
    (lambda (form rename compare)
      (let ((nation       (list-ref form 1))
            (region       (list-ref form 2))
            (customer     (list-ref form 3))
            (supplier     (list-ref form 4))
            (part         (list-ref form 5))
            (orders       (list-ref form 6))
            (lineitem     (list-ref form 7))
            (orderdate-lo (list-ref form 8))
            (orderdate-hi (list-ref form 9)))
        `(let ((lo% ,orderdate-lo)
               (hi% ,orderdate-hi))
           (begin
             (boss-eval (Name
             ...
             (boss-run
               `(Project
                  (OrderBy
                    (GroupBy
                      (Project
                        (Join (Join (Join (Join (Join (Filter ,(quote ,orders)
                                          (And (GreaterEqual o_orderdate ,lo%)
                                               (LessEqual o_orderdate ,hi%)))
      (ByName q8_cust_america)
      (Equal o_custkey c_custkey))
      ,(quote ,lineitem)
      (Equal o_orderkey l_orderkey))
      (Filter ,(quote ,part) (Equal p_type "ECONOMY ANODIZED STEEL"))
      (Equal l_partkey p_partkey))
      ,(quote ,supplier)
      (Equal l_suppkey s_suppkey))
      (ByName q8_supp_nation)
      (Equal s_nationkey sn_nationkey))
...
(define-syntax tpch-q9
  (er-macro-transformer
    (lambda (form rename compare)
      (let ((nation   (list-ref form 1))
            (supplier (list-ref form 2))
            (part     (list-ref form 3))
            (lineitem (list-ref form 4))
            (partsupp (list-ref form 5))
            (orders   (list-ref form 6)))
        `(begin
           (boss-eval (Name
           ...
           (boss-eval
             (OrderBy
               (GroupBy
                 (Project
                   (Join (Join (Join (Join (Filter ,part (Like p_name "%green%"))
      ,lineitem
      (Equal p_partkey l_partkey))
      ,partsupp
      (Equal l_suppkey ps_suppkey) (Equal l_partkey ps_partkey))
      ,orders
      (Equal l_orderkey o_orderkey))
      (Join ,supplier
      (ByName q9_nation)
...
(define-syntax tpch-q10
  (er-macro-transformer
    (lambda (form rename compare)
      (let ((orders       (list-ref form 1))
            (customer     (list-ref form 2))
            (lineitem     (list-ref form 3))
            (nation       (list-ref form 4))
            (orderdate-lo (list-ref form 5))
            (orderdate-hi (list-ref form 6)))
        `(let ((lo% ,orderdate-lo)
               (hi% ,orderdate-hi))
           (boss-run
             `(Slice
                (OrderBy
                  (Project
                    (GroupBy
                      (Project
                        (Join (Join (Join (Filter ,(quote ,orders)
                                      (And (GreaterEqual o_orderdate ,lo%)
                                           (Less o_orderdate ,hi%)))
      ,(quote ,customer)
      (Equal o_custkey c_custkey))
      (Filter ,(quote ,lineitem) (Equal l_returnflag "R"))
      (Equal o_orderkey l_orderkey))
      ,(quote ,nation)
      (Equal c_nationkey n_nationkey))
...
(define-syntax tpch-q11
  (er-macro-transformer
    (lambda (form rename compare)
      (let ((partsupp  (list-ref form 1))
            (supplier  (list-ref form 2))
            (nation    (list-ref form 3))
            (threshold (list-ref form 4)))
                  ...
                  (Filter
                    (GroupBy
                      (Project
                        (Join ,(quote ,partsupp)
      (Join ,(quote ,supplier)
      (Filter ,(quote ,nation) (Equal n_name "GERMANY"))
      (Equal s_nationkey n_nationkey))
      (Equal ps_suppkey s_suppkey))
                        ps_partkey
...
(define-syntax tpch-q12
  (er-macro-transformer
    (lambda (form rename compare)
      (let ((orders         (list-ref form 1))
            (lineitem       (list-ref form 2))
            (receiptdate-lo (list-ref form 3))
            (receiptdate-hi (list-ref form 4)))
        `(let ((lo% ,receiptdate-lo)
               (hi% ,receiptdate-hi))
           (boss-run
             `(OrderBy
                (GroupBy
                              ...
                              (And (Or (Equal l_shipmode "MAIL") (Equal l_shipmode "SHIP"))
                                   (And (Less l_shipdate l_commitdate)
                                        (And (Less l_commitdate l_receiptdate)
                                             (And (GreaterEqual l_receiptdate ,lo%)
                                                  (Less l_receiptdate ,hi%))))))
      (Equal o_orderkey l_orderkey))
                    l_shipmode
...
(define-syntax tpch-q14
  (er-macro-transformer
    (lambda (form rename compare)
      (let ((lineitem    (list-ref form 1))
            (part        (list-ref form 2))
            (shipdate-lo (list-ref form 3))
            (shipdate-hi (list-ref form 4)))
        `(let ((lo% ,shipdate-lo)
               (hi% ,shipdate-hi))
           (boss-run
             `(Project
...
(define-syntax tpch-q16
  (er-macro-transformer
    (lambda (form rename compare)
      (let ((part     (list-ref form 1))
            (partsupp (list-ref form 2))
            (supplier (list-ref form 3)))
        `(begin
           (boss-eval (Name
        ...
        `(begin
           ...
           (boss-eval
             (Slice
               (OrderBy
                 (GroupBy
                   (Join (Join (Join ,customer
      ,orders
      (Equal c_custkey o_custkey))
      (ByName q18_heavy)
      (Equal o_orderkey heavy_orderkey))
      ,lineitem
      (Equal o_orderkey l_orderkey))
...
(define-syntax tpch-q20
  (er-macro-transformer
    (lambda (form rename compare)
      (let ((part        (list-ref form 1))
            (lineitem    (list-ref form 2))
            (partsupp    (list-ref form 3))
            (nation      (list-ref form 4))
            (supplier    (list-ref form 5))
            (shipdate-lo (list-ref form 6))
            (shipdate-hi (list-ref form 7)))
        `(let ((lo% ,shipdate-lo)
               (hi% ,shipdate-hi))
             ...
             (boss-eval (Name
               (Project
                 (Filter
                   (Join (Join ,partsupp
      (ByName q20_li_qty)
      (Equal ps_partkey l_partkey) (Equal ps_suppkey l_suppkey))
      (ByName q20_forest_parts)
      (Equal ps_partkey p_partkey))
             ...
             (boss-eval
               (OrderBy
                 (Project
                   (Join (Join (Filter ,nation (Equal n_name "CANADA"))
      ,supplier
      (Equal n_nationkey s_nationkey))
      (ByName q20_qualifying_supps)
      (Equal s_suppkey ps_suppkey))
                   s_name s_address)
                 (keys s_name)))))))))
           ...
           (boss-eval
             (Slice
               (OrderBy
                 (GroupBy
                   (Project
                     (AntiJoin (Join (Join ;; Project renames l_orderkey_l back to l_orderkey for the semi-join.
                           (Project
                             (Join (ByName q21_late_li)
      (ByName q21_multi_supp_orders)
      (Equal l_orderkey l_orderkey))
